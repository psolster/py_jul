...
# TODO Для значений цвета можно использовать готовые кортежи цветов из
#  библиотеки simple_draw, например, sd.COLOR_RED.
#  Если ваши цвета вас больше устраивают, из можно оставить.
...
# TODO Для повышения надёжности работы программы нужно проверять, что вводит пользователь.
#  Сейчас, если пользователь введёт не цифру а строку символов,
#  в строке user_color = int(input(...)) произойдёт ошибка.
#  Т. к. мы ожидаем от пользователя число, то можно использовать метод isdigit
#  строки user_input: if user_input.isdigit().
#  Есть ещё один вариант − использовать в качестве ключей словаря цифры в виде строк:
#  вместо слоавря вида my_dict = {1: 'a', 2: 'b',...} сделать my_dict = {'1': 'a', '2': 'b',...}
#  Тогда всю работу можно оставить оператору in. Если user_input не является ключом словаря,
#  выводим сообщение об ошибке, не разбираясь ввёл пользователь цифру или строку.
